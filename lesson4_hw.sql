...
FROM cats c JOIN shops s ON c.shops_id = s.id; -- котики которые продаются в магазинах
SELECT c.name, s.shopname
FROM cats c LEFT JOIN shops s ON c.shops_id = s.id; -- котики все (могут быть кот. и не продаются)
-- Вывести магазин, в котором продается кот “Мурзик” (попробуйте выполнить 2 способами)
SELECT  s.*, c.name
FROM shops s JOIN cats c ON s.id = c.shops_id
WHERE  c.name = 'Murzik';
SELECT c.name, s.shopname
FROM cats c, shops s 
...
SELECT c.name, s.shopname
FROM cats c, shops s 
WHERE c.shops_id = s.id AND c.name <> 'Murzik' AND c.name <> 'Zuza';

-- Вывести название и цену для всех анализов, которые продавались 5 февраля 2020 и всю следующую неделю.
SELECT A.an_name , A.an_price 
FROM orders O
LEFT JOIN analysis A ON O.ord_an = A.an_group
WHERE O.ord_datetime > '2020-02-04 23:59:59' AND O.ord_datetime < '2020-02-13 00:00:00';

SELECT  A.an_name , A.an_price 
FROM analysis A
RIGHT JOIN (SELECT ord_an FROM orders WHERE ord_datetime > '2020-02-04 23:59:59' AND ord_datetime < '2020-02-13 00:00:00')O
