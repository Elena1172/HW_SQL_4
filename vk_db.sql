...
SELECT COUNT(*)  AS Total_likes, profiles.gender FROM likes
JOIN users ON likes.user_id = users.id
JOIN profiles ON users.id = profiles.user_id
GROUP BY gender;
...
-- (по желанию)* Пусть задан некоторый пользователь(выбираете сами, к примеру, Reuben). 
-- Из всех друзей этого пользователя найдите человека, который больше всех написал ему сообщений.
SELECT users.firstname, tabl.all_message FROM users
JOIN
(SELECT COUNT(*) AS all_message, from_user_id, to_user_id
FROM messages
JOIN users ON messages.to_user_id = users.id
WHERE users. firstname = 'Reuben'
GROUP BY from_user_id
ORDER BY COUNT(*) DESC
LIMIT 1) tabl ON users.id = from_user_id;
